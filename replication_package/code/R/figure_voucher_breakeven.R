...
## y: T2 production-value gain = 151.35 kg per HH (Table 5 Panel A) * price
## Horizontal: voucher face value 170,000 MWK
## ---------------------------------------------------------------------------
suppressPackageStartupMessages({ library(ggplot2) })

yield_effect_kg <- 151.35
yield_se        <- 72.22
voucher_cost    <- 170000
median_price    <- 750
...
g <- ggplot(df, aes(x = price, y = ret)) +
  ...
  labs(x = "Maize price (MWK / kg)",
       ...
       caption = paste0("Yield effect: 151.35 kg per household (Table 5, Panel A); ",
                        "shaded band: 95% CI.")) +
