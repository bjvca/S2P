# Generate the AIP fertilizer-use table for the manuscript.
#
# This table asks whether treatment changed fertilizer received through the
# Agricultural Input Program (AIP). It uses the same preferred specification as
# the total fertilizer-use table:
#   - unadjusted ITT columns for all crops and maize only;
#   - adjusted columns with pre-treatment controls only;
#   - categorical controls entered as factors;
#   - no agro_vis or ext_srv controls because those can respond to treatment.
#
# Unlike the total fertilizer-use table, total_qty_fert_AIP does not show an
# extreme-outlier problem. The largest observed value is 325 kg, so no case-level
# exclusion is applied here.
...
fit_spec <- function(data, rhs_terms, sample_label, column_label) {
  vars_needed <- c("total_qty_fert_AIP", "treat_num", "cluster_id_num", rhs_terms)
  vars_needed <- unique(vars_needed)
  data <- data[complete.cases(data[, vars_needed]), ]

  model <- lm(reformulate(rhs_terms, response = "total_qty_fert_AIP"), data = data)
  ...
  data.frame(
    ...
    control_mean = mean(data$total_qty_fert_AIP[data$treat == "C"], na.rm = TRUE),
    t1 = t1_beta,
    t1_se = t1_se,
    t1_p = t1_p,
    t2 = t2_beta,
...
spec_results <- rbind(
  fit_spec(df, "treat_num", "All crops", "(1)"),
  fit_spec(df, c("treat_num", preferred_controls), "All crops", "(2)"),
  fit_spec(subset(df, main_maize), "treat_num", "Maize only", "(3)"),
  fit_spec(
    subset(df, main_maize),
...
write.csv(
  spec_results,
  file.path(dir_logs, "table3_aip_fertilizer_use.csv"),
  row.names = FALSE
)
...
writeLines(table_lines, file.path(dir_tables, "table3_aip_fertilizer_use.tex"))

message("Wrote AIP fertilizer-use table to: ", file.path(dir_tables, "table3_aip_fertilizer_use.tex"))
message("Wrote AIP fertilizer-use log to: ", file.path(dir_logs, "table3_aip_fertilizer_use.csv"))
