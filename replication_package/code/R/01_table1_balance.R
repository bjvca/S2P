...
for (i in seq_along(balance_vars)) {
  ...
  fit <- lm(
    as.formula(paste(balance_vars[i], "treatment1+treatment2", sep = "~")),
    data = sampling_frame
  )
...
if (file.exists(reference_path)) {
  ...
  comparison <- df_balance - reference_balance
  comparison_path <- file.path(dir_logs, "table1_balance_comparison.csv")
  write.csv(comparison, comparison_path, row.names = FALSE)
  if (max(abs(comparison), na.rm = TRUE) > 1e-8) {
    warning("Generated balance table differs from balance_2022.Rdata; see ", comparison_path)
  } else {
    message("Generated balance table matches balance_2022.Rdata")
  }
}
