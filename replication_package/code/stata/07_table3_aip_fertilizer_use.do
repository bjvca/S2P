...
* Reproduce the AIP fertilizer-use impact table from the public endline file.
*
* Input:
...
program define post_spec
    ...
    quietly summarize total_qty_fert_AIP if e(sample) & treat == "C"
...
foreach v in treat_num cluster_id_num total_qty_fert_AIP hh_size hh_age dist_agro plot_siz seed_typ_num {
...
quietly regress total_qty_fert_AIP i.treat_num `fert_controls', vce(cluster cluster_id)
post_spec, label("(2)") sample("All crops") controls("Yes") handle(`handle')

quietly regress total_qty_fert_AIP i.treat_num if main_crp == "MAIZE", vce(cluster cluster_id)
post_spec, label("(3)") sample("Maize only") controls("No") handle(`handle')

quietly regress total_qty_fert_AIP i.treat_num `fert_controls' if main_crp == "MAIZE", ///
...
use "`results'", clear
export delimited using "`out_logs'/table3_aip_fertilizer_use_stata.csv", replace
...
file open tex using "`out_tables'/table3_aip_fertilizer_use_stata.tex", write replace
